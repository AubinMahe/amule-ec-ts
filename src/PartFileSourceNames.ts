...
/**
 * One entry of DownloadFile.sourceNames - see DownloadFile's class doc for what an absent `name`
 * or a `count` of 0n mean.
 */
...
/**
 * Decodes an EC_TAG_PARTFILE_SOURCE_NAMES container off `tag` - a single response's raw delta, see
 * DownloadFile's class doc for the protocol this reflects.
 */
 ...
 * connection already been told" per file, shared across every request type that touches that
 * file's EC_TAG_PARTFILE encoding - confirmed against ExternalConn.cpp:
 * Get_EC_Response_GetDownloadQueue and Get_EC_Response_GetSharedFiles both pass the same
 * per-connection CFileEncoderMap (ExternalConn.cpp:3592-3593/3624-3625), and a partial file that's
 * also shared reuses the very same CPartFile_Encoder across both rather than getting a second one
 * for the shares pass (ExternalConn.cpp:355-364, "A partfile appears in both lists ... Leaving the
 * new entries unmerged would ... build a second encoder for the same ECID"). That state isn't
 * reset by a fresh EC_DETAIL_CMD request either: CPartFile_Encoder::ResetEncoder()
 * (ExternalConn.cpp:3359-3363) resets gap/req status but never touches m_sourcenameItemMap.
 *
 ...
 * connection - its own earlier fetch, another service's fetch
 * (Downloads.fetch()/SharedFiles.fetch()/ Update.fetch() - Get_EC_Response_GetUpdate also takes
 * the same m_FileEncoder, ExternalConn.cpp:3641 - all three end up encoding the very same object
 * for a given ecid), or a push notification. Keyed by connection instance (a WeakMap, so an entry
 * never outlives the connection that produced it) rather than by which class asked: every one of
 * them feeds and reads the same cache through resolveSourceNames() below, so whichever happens to
 * reach the daemon first, the others still see it on their own next call - no protocol awareness
 * required from any of them.
 */
 ...
 * been told about `ecid`, returning the accumulated total. Falls back to the bare per-tag delta,
 * with no accumulation, when `connection` or `ecid` is undefined (a removal notification carries
 * no ecid; tests decoding a single tag in isolation have no connection at all) - see
 * mergeSourceNames()'s doc for why that's still a safe reading on its own.
 */
 ...
 * so a later ecid reuse can't inherit a stale history.
 */
